...
current_week="2019W6" #Change to match the current week, but it won't break the code either way. 
...
=======
# For 2019 data, we can also grab facility names.  #EMILY STILL NEED TO RE-RUN THIS AND GRAB ADULT, KID, AND RAPID TEST DATA! JUST HAVE ADULT RIGHT NOW.           
#---------------------------------------------------------------  
#Set as one week if you only need the most recent data 
#weeks = "2019W6"

#Set as a list if you want to grab a range of weeks                 
weeks = list()
for year in {2019}:
    for week in range(6):  
        week_string = str(year) + "W" + str(week+1)
        weeks.append(week_string)
>>>>>>> d18fbaa8b9c3924ed9092755702da23deb65d611

patient_levels = {
        "adult": '//*[@id="app"]/article/div/div/div[1]/ul/li[1]',
...
#---------------------------------------------------------------
# For the weeks specified above, pull the list of facilities
...
for week in weeks:
    ...
    facility_save = final_dir + str(week) + "facility_level.csv"
    commodity_save = final_dir + str(week) + "commodities_overview.csv"
    ...
    if os.path.isfile(commodity_save) and overwrite_data != "yes":
        ...
        aggregated_facilities = pd.DataFrame()
        aggregated_commodities = pd.DataFrame()
        #Break this down by adult, pediatric, and RTKS. 
        for level in patient_levels:
            print("Downloading " + str(level))
            ...
            if (len(commodities)!=0): 
                ...
                if level == "adult":
                    aggregated_commodities = commodities
                else:
                    aggregated_commodities = aggregated_commodities.append(commodities)
                
                #Grab facility name information where we have it. 
                for index, row in commodities.iterrows(): 
                    ...
                    if index == 0:
                        aggregated_facilities = facilities
                    else:
                        aggregated_facilities = aggregated_facilities.append(facilities)
                ...
                aggregated_facilities.to_csv(facility_save)
                time.sleep(3)
                
            #Write the aggregated files
            aggregated_commodities.to_csv(commodity_save)
            time.sleep(3)
...
#---------------------------------------------------------------
# Archiving the code used to pull 2016-2018 data (does not need to be rerun). 
#---------------------------------------------------------------
#
##---------------------------------------------------------------
##Set the names of the districts and weeks you want to pull here. 

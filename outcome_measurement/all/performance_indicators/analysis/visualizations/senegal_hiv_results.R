...
country = "sen"
diseases = c("hiv")
main_only <- c("impact_outcome_indicators_main", "coverage_indicators_main")
...
# subset data
DT <- data
DT <- DT[loc_name==country & disease %in% diseases & pudr_sheet %in% main_only]
...
pudrs = unique(DT2$file_name)
aplots = list()
i=1
for(p in pudrs) {
   aplots[[i]] = ggplot(DT3[file_name==p], aes(x=brief_description_code, y=value, fill=variable)) +
      geom_bar(stat="identity",position = "dodge", alpha=.3) +
      labs(fill= "Values", title=paste0("HIV Testing indicators ", DT3[file_name==p]$date), y="Value (%)", x="Indicator", caption=paste0("Source: ", unique(DT3[file_name==p]$file_name))) +
      theme_bw()+
      coord_flip()+
...
i=1
for(p in pudrs) {
   plots[[i]] = ggplot(DT2[file_name==p], aes(x=brief_description_code, y=ihme_result_achievement_ratio, col=target_met)) +
      geom_point() +
      labs(col= "Target Met", title=paste0("HIV Testing Achievement ", DT2[file_name==p]$date), y="Achievement Ratio", x="Indicator", caption=paste0("Source: ", unique(DT2[file_name==p]$file_name))) +
...
pdf("J:/Project/Evaluation/GF/outcome_measurement/multi_country/performance_indicators/pudr_indicator_extraction/visualizations/sen_hiv_results.pdf", height = 10, width = 15)
aplots[[1]]
aplots[[2]]
aplots[[3]]
aplots[[4]]
...
# save data-set used
#outFile <- "J:/Project/Evaluation/GF/outcome_measurement/multi_country/performance_indicators/pudr_indicator_extraction/prepped_data/sen_hiv_results.csv"

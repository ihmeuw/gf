...
# PLOTS 
#-----------------------------------
pdf("J:/Project/Evaluation/GF/outcome_measurement/multi_country/performance_indicators/pudr_indicator_extraction/visualizations/absorption_indicators_comparison.pdf", width=10, height=8)
#First, run a scatter plot of achivement ratio vs. absorption percentage. 
ggplot(dt[start_date>="2018-01-01"], aes(x=ihme_result_achievement_ratio, y=absorption, color=disease)) + 
  geom_point() + 
  theme_bw(base_size=18) + 
  facet_wrap(~start_date) + 
  labs(title="Absorption vs. Achievement Ratio", x="Achievement Ratio", y="Absorption (%)", color="")
...
for (c in c('DRC', 'SEN', 'UGA')){
  for (d in unique(dt$disease)) {
    ...
    if (nrow(subset)>0){
      p = ggplot(subset, aes(x=ihme_result_achievement_ratio, y=absorption)) + 
        geom_point() + 
        theme_bw(base_size=18) + 
        facet_wrap(~start_date) + 
        labs(title=paste0("Absorption vs. Achievement Ratio for ", c, " ", d), x="Achievement Ratio", y="Absorption (%)", color="")
      print(p) 
    } 
  } 
} 

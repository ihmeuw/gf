...
band3 = band_to_year[year==y3, band]

# output file
graphFile = paste0(outDir, 'HIV_Prevalence_', timestamp, '_', y1,'_',y3,'_new.pdf')
...
# load raster data
rasterData1 = raster(inFile, band=band1)
rasterData2 = raster(inFile, band=band2)
rasterData3 = raster(inFile, band=band3)
...
rasterDataCOD2 = crop(rasterData2, extent(mapCOD))
rasterDataCOD3 = crop(rasterData3, extent(mapCOD))

# mask to the country borders
rasterDataCOD1 = mask(rasterDataCOD1, mapCOD)  
rasterDataCOD2 = mask(rasterDataCOD2, mapCOD) 
rasterDataCOD3 = mask(rasterDataCOD3, mapCOD) 

# mask the bodies of water
rasterDataCOD1 = mask(rasterDataCOD1, lakes, inverse=TRUE)
rasterDataCOD2 = mask(rasterDataCOD2, lakes, inverse=TRUE)
rasterDataCOD3 = mask(rasterDataCOD3, lakes, inverse=TRUE)

# convert to data tables
dataCOD1 = data.table(as.data.frame(rasterDataCOD1, xy=TRUE))
dataCOD2 = data.table(as.data.frame(rasterDataCOD2, xy=TRUE))
dataCOD3 = data.table(as.data.frame(rasterDataCOD3, xy=TRUE))
...
setnames(dataCOD2, c('x','y','prev'))
setnames(dataCOD3, c('x','y','prev'))
...
dataCOD = rbind(dataCOD1, dataCOD2, dataCOD3)

pdf(paste0(export_dir_cod, '/hiv_prevalence_2000_2017.pdf'), width=12, height=9)

# store maps
ggplot(dataCOD, aes(y=y, x=x, fill=prev*100)) +
  geom_tile() +
  geom_path(data=shapeDataCOD, aes(x=long, y=lat, group=group),
            color=border, size=.05, inherit.aes=FALSE) +
  scale_fill_gradientn('HIV Prevalence (%)', colors=reds,
...
# rbind the bands together
dataUGA = rbind(dataUGA1, dataUGA2)

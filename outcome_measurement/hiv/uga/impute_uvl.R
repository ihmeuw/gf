...
ids <- c('facility_id', 'date')

males <- uganda_vl[sex=='Male', .(male_pts=sum(patients_received)), by=.(facility_id, date)]
uganda_vl <- merge(uganda_vl, males, by=ids, all.x=TRUE)
uganda_vl[sex=='Female', male_pts:=0]
uganda_vl[sex=='Unknown', male_pts:=0]

females <- uganda_vl[sex=='Female', .(female_pts=sum(patients_received)), by=.(facility_id, date)]
uganda_vl <- merge(uganda_vl, females, by=ids, all.x=TRUE)
uganda_vl[sex=='Male', female_pts:=0]
uganda_vl[sex=='Unknown', female_pts:=0]

unks <- uganda_vl[sex=='Unknown', .(unk_pts=sum(patients_received)), by=.(facility_id, date)]
...
known <- uganda_vl[,.(known_pts=sum(female_pts+male_pts)), by=.(facility_id, date)]
uganda_vl <- merge(uganda_vl, known, by=ids, all.x=TRUE)

# apply the sex ratio of females to the unknown patients
fems <- uganda_vl[ , .(fems=sum(female_pts)), by=.(facility_id, date)]
uganda_vl <- merge(uganda_vl, fems, by=ids, all.x=TRUE)
uganda_vl[ , ratio_fems:=(fems/known_pts)]
uganda_vl[ , new_fems:=(unk_pts*ratio_fems)]

# apply the sex ratio of males to the unknown patients
gents<- uganda_vl[ , .(gents=sum(male_pts)), by=.(facility_id, date)]
uganda_vl <- merge(uganda_vl, gents, by=ids, all.x=TRUE)
uganda_vl[ , ratio_gents:=(gents/known_pts)]
uganda_vl[ , new_fems:=(unk_pts*ratio_gents)]




fems <- uganda_vl[ , .(percent_female=(female_pts/known_pts)), by=.(facility_id, date)]



uganda_vl <- merge(uganda_vl, fems, by=ids, all.x=TRUE)

uganda_vl[ ,.(new_fems=(unk_pts*percent_female)), by=.(facility_id, date)]


# ----------------

uganda_vl[ ,new_fems=(unk_pts*percent_female)]
...
test_graph <- uganda_vl[ ,.(male_pts=sum(male_pts), female_pts=sum(female_pts),
                            unk_pts=sum(unk_pts)),by=date]
test_graph <- melt(test_graph, id.vars='date')
test_graph$variable <- factor(test_graph$variable, levels=c("male_pts", "female_pts", "unk_pts"),
                              labels=c("Male", "Female", "Unknown"))
View(test_graph)

ggplot(test_graph, aes(x=date, y=value, color=variable)) +  
  geom_point() + 
  geom_line() + theme_bw() + 
  labs(title="Monthly patients submitting samples by sex", x="Date", y="Count")

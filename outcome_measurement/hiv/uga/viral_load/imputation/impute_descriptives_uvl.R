...
#-----------------------
# check to make sure the imputation ran accurately
...
avl <- avl[, lapply(.SD, exp), by=c('facility_id', 'facility_name', 'dhis2name', 'sex', 'date', 'district_id', 'dist_name', 'level', 'imputed', 
                                    'missing_males', 'missing_females', 'ratio1', 'patients_received0', 'samples_received0', 'rejected_samples0', 
                                    'dbs_samples0', 'samples_tested0', 'valid_results0', 'suppressed0', 'imputation_number', 'ratio'), .SDcols=expVars]

#--------------------
# back transform the identified 0s back to 0; all 0 values should be 1 before transformation
...
# district ratio - ratios that were = 0/0 should be NA
avl[valid_results0==1 & suppressed0==1, ratio:=NA]
# check that when valid results and suppressed are 0, the ratio is missing
avl[valid_results==0 & suppressed==0, unique(ratio)]
...
saveRDS(avl, paste0(dir,"/imputed_offset_prepped.rds"))
...
avl_mean <- avl[, .(patients_received=mean(patients_received),
      ...
      ratio_lower=quantile(ratio, 0.025, na.rm=T),
      ...
      ratio_upper=quantile(ratio, 0.975, na.rm=T)),  by=byVars]
...
ratio_loop <- avl_mean[  , .(samples_received, samples_received_upper, samples_received_lower,
                             ...
                             ratio_upper, ratio_lower), by=idVars]
...
for(f in unique(ratio_loop$facility_id)) {
  ...
  list_of_plots[[i]] <- ggplot(ratio_loop[facility_id==f], aes(y=mean, x=date, ymin=lower, ymax=upper)) + 
    ...
    geom_line(alpha=0.5) + 
    facet_wrap(~variable, scales='free_y') +
    labs(title=name2, x="Date") +
    theme_bw()
  
  if (!all(is.na(ratio_loop[facility_id==f]$lower))) { 
      list_of_plots[[i]] = list_of_plots[[i]] + geom_errorbar(color='red', alpha=0.6)
  }
...
avl_mean_sex <- avl[, .(patients_received=mean(patients_received),
                    ...
                    valid_results_upper=quantile(valid_results, 0.975),
                    suppressed_upper=quantile(suppressed, 0.975),
                    ratio_upper=quantile(ratio, 0.975, na.rm=T)),
...
#-----------------------------------------------------
# TIME 2
# prep the data for loops and visualize, using the squared time imputation 
...
avl <- readRDS(paste0(dir,"/imputed_offset_time2.rds"))
...
pdf(paste0(dir,'/webscrape_agg/outputs/imputed/offset_ls_facility_time2.pdf'), height=6, width=9)
...
pdf(paste0(dir,'/webscrape_agg/outputs/imputed/offset_ls_facility_sex_time2.pdf'), height=6, width=9)
...
#-----------------------------------------------------
# TIME 3
# prep the data for loops and visualize, using the cubed time imputation 
...
avl <- readRDS(paste0(dir,"/imputed_offset_time3.rds"))
...
pdf(paste0(dir,'/webscrape_agg/outputs/imputed/offset_ls_facility_time3.pdf'), height=6, width=9)

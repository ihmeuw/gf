...
dt_k = dt[ ,.(percent_tests, percent_arvs, test_slope, arv_slope)]
#----------------------------------------
...
for (x in c(2:10)) {
  ...
  dt[ ,centroid_x_slope:=mean(test_slope, na.rm=T), by=kcluster]
  dt[ ,centroid_y_slope:=mean(arv_slope, na.rm=T), by=kcluster]
...
pdf(paste0(dir, 'k_means_outputs/percent_arv_tests_slope_2014_2019.pdf'),height=9, width=18)

...
#------------------------
# merge in 2018 totals

df18 = df[year==2018, .(facility, tests2018 = test_kits, arvs2018 = arvs)]
dt = cbind(dt, df18, by='facility', all.x=T)

#----------------------------------------
# create a matrix for cluster analysis

dt_k = dt[ ,.(percent_arvs, percent_tests, tests2018, arvs2018)]
#----------------------------------------
...
for (x in c(2:10)) {
  ...
  dt[ ,centroid_x_slope:=mean(percent_tests, na.rm=T), by=kcluster]
  dt[ ,centroid_y_slope:=mean(percent_arvs, na.rm=T), by=kcluster]
  dt[ ,slope_label:=paste0(round(centroid_x_slope, 1), ", ", round(centroid_y_slope, 1)), by=kcluster]
  
  # mark the centroids for labeling
  dt[ ,centroid_x:=mean(tests2018, na.rm=T), by=kcluster]
  dt[ ,centroid_y:=mean(arvs2018, na.rm=T), by=kcluster]
  ...
  list_of_plots[[i]] = ggplot(full_data[total_clusters==x],
                              aes(x=tests2018, y=arvs2018, color=factor(kcluster)))+
    ...
    labs(x = "Weeks out of tests, 2018",
         y = "Weeks out of ARVs, 2018", color='Clusters',
         title="Weeks out of test kits and ARVs, 2018",
  ...
  list_of_plots_slope[[i]] = ggplot(full_data[total_clusters==x],
    ...
    labs(x = "Percent of time out of tests",
         y = "Percent of time out of ARVs", color='Clusters',
         subtitle=paste0('Number of clusters = ', x))+
    theme(text=element_text(size=18))
...
pdf(paste0(dir, 'k_means_outputs/percent_arvs_tests_2018_total_2014_2019.pdf'),height=9, width=18)
...
plot_ly(full_data[total_clusters==4],
        x = ~percent_tests, y = ~percent_arvs, z = ~tests2018, color = ~factor(kcluster),

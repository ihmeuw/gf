...
dt = merge(sigpro_pb_tvc, sigpro_its, by = c('codigounico', 'age', 'gender', 'date'), suffixes = c(".pbtvc", ".its"), all = TRUE)


#Review variables between two datasets 
dt["numeroinforme.pbtvc" != "numeroinforme.its"]
...
dt[ ,range(date, na.rm=T), by=set]

#Number of unique identifiers by dataset
dt[, length(unique(codigounico)), by=set]

#------------------------------------------------------
# Data quality concerns 
#------------------------------------------------------
dt[is.na(date)] # there are 16 missing dates 
dt[is.na(codigounico)] #No missing IDs here. 
dt[ ,unique(tipoactividad)]
dt[, unique(codgrupo)]
dt[, unique(tratado)]
dt[, unique(lubrisachet)]
dt[, unique(codproyecto), by='set']

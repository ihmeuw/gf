...
uganda_vl[,facility_id, by=facility_id] # 2040 facilities

# create a version of uganda_vl_long without rejected samples - only relevant graphs
# use for checking for missing data
uvl_long <- uganda_vl_long[variable!="rejected_samples"]

# label the variables for graph titles and put the graphs in an intuitive order
uvl_long$variable <- factor(uvl_long$variable, 
       levels=c("samples_received", "dbs_samples", "patients_received", "total_results", "valid_results", "suppressed"), 
       labels=c("Samples Received","DBS Samples", "Patients","Total Results", "Valid Results", "Suppressed"))

table(uvl_long$variable)

# create a single plot to test
ggplot(uvl_long[facility_id==2], aes(y=value, x=date, color=sex)) + 
  geom_point() + 
  geom_line(alpha=0.5) + 
...
pdf('C:/Users/ccarelli/districts.pdf', height=6, width=9)
...
# --------------------
# create plots by facility by looping over all facilities, export as a PDF
...
list_of_plots = NULL
i=1
for(f in unique(uvl_long$facility_id)) {
  # make your graph
  
  list_of_plots[[i]] <- ggplot(uvl_long[facility_id==f], aes(y=value, x=date, color=sex)) + 
    geom_point() + 
    geom_line(alpha=0.5) + 
    facet_wrap(~variable, scales='free_y') + xlab("Date") + ylab("Count") + theme_bw()
...
list_of_plots = NULL
i=1
for(f in unique(uvl_long$district_id)) {
  # make your graph
  
  list_of_plots[[i]] <- ggplot(uvl_long[district_id==f], aes(y=value, x=date, color=factor(sex))) + 
    geom_point() + 
    geom_line(alpha=0.5) + 
    facet_wrap(~variable, scales='free_y') +
    theme_bw() + xlab("Date") 
...
# create plots by district, looping over every district

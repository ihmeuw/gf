...
#------------------
{
  ...
  #Make values numeric 
  ghe_tb[, value:=gsub(",", "", value)]
  ghe_tb[, value:=as.numeric(value)]
  ...
  ghe_tb[is.na(value), .(ihme_loc_id, year_id)][order(ihme_loc_id, year_id)] #Looks like nothing after 2010. 
  ...
  ghe_tb = ghe_tb[, .(expenditure=sum(value, na.rm=T)), by=c('ihme_loc_id', 'year_id', 'value_code', 'source_type')]

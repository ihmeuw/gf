...
gtm_merge <- gtm_merge[, .(fileName, correct_bug_sum, budget, expenditure, start_date, end_date, data_source.x)]
...
uga_merge <- uga_merge[, .(fileName, correct_bug_sum, budget, expenditure, start_date, end_date, data_source.x)]
...
cod_merge$correct_bug_sum <- substring(cod_merge$correct_bug_sum, 2)
cod_merge$correct_bug_sum <- gsub("[[:punct:]]", "", cod_merge$correct_bug_sum)
cod_merge$correct_bug_sum <- as.numeric(cod_merge$correct_bug_sum)

cod_merge$budget = round(cod_merge$budget)

cod_merge <- cod_merge[, .(fileName, correct_bug_sum, budget, expenditure, start_date, end_date, data_source.x)]
...
print(paste0("Testing ", round(gtm_tests[format == "pudr", .N]/gtm_filelist[data_source == "pudr", .N]*100, 2), "% of PUDRs and ", 
             round(gtm_tests[format != "pudr", .N]/gtm_filelist[data_source != "pudr", .N]*100, 2), "% of budgets in Guatemala"))
print("Testing % of active grants and % of not active grant Guatemala")
print("...")

print(paste0("Testing ", round(uga_tests[type == "pudr", .N]/uga_filelist[type == "pudr", .N]*100, 2), "% of PUDRs and ", 
             round(uga_tests[type != "pudr", .N]/uga_filelist[type != "pudr", .N]*100, 2), "% of budgets in Uganda"))
print(paste0("Testing ", round(uga_tests[status == "active", .N]/uga_filelist[status == "active", .N]*100, 2), "% of active grants and ", 
             round(uga_tests[status != "active", .N]/uga_filelist[status != "active", .N]*100, 2), "% of not active grants in Uganda"))
print("...")

print(paste0("Testing ", round(cod_tests[type == "pudr", .N]/cod_filelist[type == "pudr", .N]*100, 2), "% of PUDRs and ", 
             round(cod_tests[type != "pudr", .N]/cod_filelist[type != "pudr", .N]*100, 2), "% of budgets in DRC"))
print(paste0("Testing ", round(cod_tests[status == "active", .N]/cod_filelist[status == "active", .N]*100, 2), "% of active grants and ", 
             round(cod_tests[status != "active", .N]/cod_filelist[status != "active", .N]*100, 2), "% of not active grants in DRC"))

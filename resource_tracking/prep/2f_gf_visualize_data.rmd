...
What are the data gaps between GOS and budgets, expenditures? 
```{r, echo=TRUE}
budgets_only = budgets[data_source=='fpm']
gos_budgets = budgets[data_source=='gos' & grant%in%budgets_only$grant]

expenditures_only = expenditures[data_source=='pudr']
gos_expenditures = expenditures[data_source=='gos' & grant%in%expenditures_only$grant] 

budgets_only[, .(min_start_date = min(start_date)), by='grant']
gos_budgets[, .(max_start_date = max(start_date)), by='grant']

expenditures_only[, .(min_start_date = min(start_date)), by='grant']
gos_expenditures[, .(max_start_date = max(start_date)), by='grant']

```
Look at the unique values of each variable in the data that's not numeric. 
```{r, echo=FALSE, include=FALSE}
#stopifnot(sort(names(budgets))==sort(names(expenditures)))
str(budgets)

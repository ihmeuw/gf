...
prep_modular_approach_pudr =  function(dir, inFile, sheet_name, start_date, period, qtr_number) {
  ...
  gf_data = gf_data[, .(budget=sum(budget, na.rm=T), expenditure=sum(expenditure, na.rm=T), lfa_exp_adjustment=sum(lfa_exp_adjustment, na.rm=T)), by=c('module', 'intervention')]

...
# PURPOSE: Prep DRC provincial budgets - based off of general detailed budget function. 
# DATE: Modified October 2019. 

# The difference between these files and the general budgets are that they're SR specific and subnational, 
# and they only cover 2019 instead of 2018-2020. 
...
prep_general_detailed_budget = function(dir, inFile, sheet_name, start_date, period, qtr_num, language) {
  ...
  # qtr_num = file_list$qtr_number_financial[i]
  ...
  old_qtr_names = c()
  for (i in 1:quarters){ #These files only have quarters 5-8 (year 2, 2019). 
    old_qtr_names[i] = paste0(qtr_text, " t", i+4)
  }
  
  new_qtr_names = c()
  for (i in 1:quarters){ #These files only have quarters 5-8 (year 2, 2019). 
    new_qtr_names[i] = paste0("budget_q", i+4)
  }
  ...
  #Drop extra row with "module/intervention" 
  gf_data = gf_data[!module=="Module"]

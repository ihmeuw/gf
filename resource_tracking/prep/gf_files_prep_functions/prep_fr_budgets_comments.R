...
prep_fr_budgets_comments = function(dir, inFile, sheet_name, start_date, period, qtr_num, language, file_iteration) {
  ...
  KEEP_COLS = 6 #Number of columns that are extracted before the budget columns (Module, intervention, activity, cost category, and implementer, and comments)
  ...
  # 1. Subset columns.
  #-------------------------------------
  #Find the correct column indices based on a grep condition. (Want to keep module, intervention, budget, cost category, and activity description and comments column)
  ...
  # grab comments column
  if (language=="esp"){
    comments_col = grep("justificacion", names)
  } else if (language%in%c("eng","fr")){
    comments_col = grep("justification", names)
  }
  
  stopifnot(length(comments_col)==1)
  
  #Grab all budget rows 
  budget_cols = grep(qtr_text, names)
  
  total_subset = c(module_col, intervention_col, activity_col, cost_category_col, implementer_col, comments_col, budget_cols)
  ...
  if (language == "eng"){
    if ('recipient'%in%names){
      old_names = c('activity description', 'cost input', 'recipient','justification/comments', old_qtr_names)
      new_names = c('activity_description', 'cost_category', 'implementer','comments', new_qtr_names)
    } else {
      old_names = c('activity description', 'cost input', 'justification/comments', old_qtr_names)
      new_names = c('activity_description', 'cost_category', 'comments', new_qtr_names)
    }
  } else if (language == "fr") {
    if (inFile == "COD_M_SANRU NMF2 ANNEXE FINANCES  FORECAST  budget revisé 2018.xlsx"){ # added in to check specific file
      old_names = c("intervention revise", "description de l'activite", "element de cout", "entite de mise en œuvre", "justification / commentaires", old_qtr_names)
      new_names = c("intervention", "activity_description", "cost_category", "implementer", "comments", new_qtr_names)
    } else if ("maitre d'œuvre"%in%names){
      old_names = c("description de l'activite", "element de cout", "maitre d'œuvre", "justification / commentaires", old_qtr_names)
      new_names = c("activity_description", "cost_category", "implementer", "comments", new_qtr_names)
    } else if ("recipiendaire"%in%names){
      old_names = c("description de l'activite", "element de cout", "recipiendaire", "justification / commentaires", old_qtr_names)
      new_names = c('activity_description', 'cost_category', "implementer","comments" , new_qtr_names)
    } else {
      old_names = c("description de l'activite", "element de cout", "justification / commentaires", old_qtr_names)
      new_names = c('activity_description', 'cost_category', "comments", new_qtr_names)
    } 
  } else if (language == "esp"){
    if ('implementador'%in% names){
      old_names = c('modulo', 'intervencion', 'descripcion de la actividad', 'categoria de gastos', "implementador", "justificacion / comentarios", old_qtr_names)
      new_names = c('module', 'intervention', 'activity_description', 'cost_category', "implementer", "comments",  new_qtr_names)
    } else if ('entidad ejecutora'%in%names){
      old_names = c('modulo', 'intervencion', 'descripcion de la actividad', 'categoria de gastos', "entidad ejecutora", "justificacion / comentarios", old_qtr_names)
      new_names = c('module', 'intervention', 'activity_description', 'cost_category', "implementer", "comments", new_qtr_names)
    } else{
      old_names = c('modulo', 'intervencion', 'descripcion de la actividad', 'categoria de gastos', "receptor", "justificacion / comentarios", old_qtr_names)
      new_names = c('module', 'intervention', 'activity_description', 'cost_category', "implementer", "comments", new_qtr_names)
    }
  }
...
# check empty module and interventions
  ...
  budget_dataset = melt(gf_data, id.vars = c('module', 'intervention', 'activity_description', 'cost_category', 'implementer', "comments"), 
                        value.name = "budget")
  ...
  check_names = c('module', 'intervention', 'cost_category', 'activity_description', 'budget', 'start_date', 'implementer', "comments")

...
if (prep_files){
  ...
  final_expenditures = final_expenditures[, .(budget=sum(budget, na.rm=T), expenditure=sum(expenditure, na.rm=T), disbursement=sum(disbursement, na.rm=T)), 
                                   by=byVars]
  #Instead of this, almost need to split into a dataset of just each grant and grant period. 
  #Need to find duplicates in quarter, and subtract the later value off of the earlier value. 
  grants = unique(final_expenditures[, .(grant, grant_period)])
  for (i in 1:nrow(grants)){
    subset = final_expenditures[]
  }

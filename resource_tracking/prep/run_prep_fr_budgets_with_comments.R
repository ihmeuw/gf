...
source("./resource_tracking/prep/gf_files_prep_functions/prep_fr_budgets_comments.R", encoding="UTF-8")
...
# subset to only guatemala files
file_list = file_list[loc_name=="gtm" & grant_period=="2021-2023"]
...
for(i in 1:nrow(file_list)){
  ...
  tmpData = do.call(prep_fr_budgets_comments, args)
  ###
...
# save raw output
# ----------------------------------------------
write.csv(prepped_frs, paste0(box, 'tableau_data/raw_extracted_gtm_nfm3_budgets_with_comments.csv'), row.names = FALSE)
# ----------------------------------------------
...
# ----------------------------------------------
prepped_frs <- fread(paste0(box, 'tableau_data/raw_extracted_gtm_nfm3_budgets_with_comments.csv'))
...
# NFM2 modular framework--there are no NFM2 data here
# 
...
mapped_data <- merge(raw_data2, module_map, by=mergeVars, all.x = TRUE)

dropped_mods2 <- mapped_data[is.na(mapped_data$gf_module), ]
...
mapped_data$comments <- str_replace_all(mapped_data$comments, "[^[:alnum:]]", " ")
# ----------------------------------------------
...
keep_cols = c('file_name', 'loc_name', 'grant', 'gf_module', 'gf_intervention', 'comments',
...
# # save data
write.csv(fr_budgets, paste0(box, "tableau_data/guatemala_nfm3_budgets_all.csv"))

...
{
  gos_data = fread(paste0(gos_raw, "Expenditure_at_InterventionSDA_Level_2015to17.csv"))
  names = c('department', 'region', 'country', 'grant', 'ip_name', 'grant_start_date', 'grant_end_date', 'module', '2015', '2016', '2017')
  names(gos_data) = names
  
  #Drop the first two rows of data after resetting names 
  gos_data = gos_data[3:nrow(gos_data), ]
  
  #Create grant period 
  gos_data[, grant_period:=paste0(year(grant_start_date), "-", year(grant_end_date))]
  
  #Only keep the columns you want 
  gos_data = gos_data[, -c('department', 'region', 'ip_name', 'grant_start_date', 'grant_end_date')]
  
  #Only keep IHME countries 
  gos_data = gos_data[country%in%c('Congo (Democratic Republic)', 'Guatemala', 'Senegal', 'Uganda')]
  
  #Drop out total row
  gos_data = gos_data[!grep("Total", module)]
  
  #Melt data 
  gos_melt = melt(gos_data, id.vars=c('country', 'grant', 'module'))
  setnames(gos_melt, c('variable', 'value'), c('year', 'expenditure'))
  
  #Fix financial variables, and create a date variable 
  gos_melt[, expenditure:=gsub(",", "", expenditure)]
  gos_melt[, expenditure:=as.numeric(expenditure)]
  
  # gos_melt[, start_date:=as.Date(paste0("01-01-", year), format="%m-%d-%Y")]
...
gms_data = gms_data[, .(expenditure=sum(expenditure, na.rm=T), budget=sum(budget, na.rm=T)), by=c('country', 'grant', 'module', 'disease', 'year') ]
gms_data = gms_data[year<=2014]
totalGos <- rbind(gms_data, gos_melt, fill=T)

...
mappedUga <- data.table(read.csv("J:/Project/Evaluation/GF/resource_tracking/uga/prepped/prepped_uga_data.csv"))
mappedUga <- mappedUga[!is.na(budget)]
uga_codebook <- data.table(read_excel("J:/Project/Evaluation/GF/resource_tracking/RT_Codebook.xlsx"))
# setwd("where the FPM budgets are stored")
...
knitr::include_graphics("J:/Project/Evaluation/GF/resource_tracking/uga/gf/fpm/fpmimage.png")
```
...
knitr::include_graphics("J:/Project/Evaluation/GF/resource_tracking/uga/gf/fpm/pudrimage.png")
```
...
```{r table, echo=FALSE}
knitr::kable(mappedUga[1:5,], caption = "Subset of the Uganda RT Database")
```
...
knitr::include_graphics("J:/Project/Evaluation/GF/resource_tracking/uga/gf/fpm/githubimage.png")
```
...
knitr::include_graphics("J:/Project/Evaluation/GF/resource_tracking/uga/gf/fpm/ugabasecampimage.png")
```

You only need to download the "fpm" and "pudrs" folders onto your local drive - but make sure to save them in the same place (perhaps under the same folder name). If they are stored in different places on your local drive, you will need to modify the R code to accomodate this, since the R code assumes that both folders share a filepath. 
...
The "prep_pudr_uga" file cleans the PUDR files we receive from the LFA. This script should flexible enough to handle PUDRs that are not all formatted the same, but may need to augmented if we receive a PUDR that has a different format. 

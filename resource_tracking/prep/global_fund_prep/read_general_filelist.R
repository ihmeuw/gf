...
for(i in 1:nrow(file_list)){ 
  ...
  } else if(file_list_data_source[i] == 'fpm' & file_list$sheet[i]=='Detailed Budget') #Emily are there other sheet names that can work with this function? 
    tmpData <- prep_detailed_budget(file_dir, file_list$file_name[i], as.character(file_list$sheet[i]), 
                                            file_list$start_date[i], file_list$disease[i], file_list$period[i], file_list$grant[i],
                                            file_list$primary_recipient[i], file_list$data_source[i])
  } else if(file_list_data_source[i] == 'fpm' & file_list$sheet[i]=='Budget summary GF') #Emily are there other sheet names that can work with this function? 
    tmpData <- prep_summary_budget(file_dir, file_list$file_name[i], as.character(file_list$sheet[i]), 
                                        file_list$start_date[i], file_list$disease[i], file_list$period[i], file_list$grant[i],
                                        file_list$primary_recipient[i], file_list$data_source[i])
  } else {

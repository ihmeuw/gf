...
current_uga = ggplot(sum_current_grants[grant%in%current_uga_grants & year>=2015], aes(x=year, y=budget, fill=grant)) + 
  geom_bar(stat="identity")+
  facet_wrap(~grant, scales='free_y')+
  theme_bw() + labs(title="Sum of budget per year, for currently active grants in Uganda", x="Year", y="Budget, with free-y scales", 
                    caption="*Years limited to 2015-present")

current_gtm = ggplot(sum_current_grants[grant%in%current_gtm_grants & year>=2015], aes(x=year, y=budget, fill=grant)) + 
  geom_bar(stat="identity")+
  facet_wrap(~grant, scales='free_y')+
  theme_bw() + labs(title="Sum of budget per year, for currently active grants in Uganda", x="Year", y="Budget, with free-y scales", 
                    caption="*Years limited to 2015 - present")

current_cod = ggplot(sum_current_grants[grant%in%current_cod_grants & year>=2015], aes(x=year, y=budget, fill=grant)) + 
  geom_bar(stat="identity")+
  facet_wrap(~grant, scales='free_y')+
  theme_bw() + labs(title="Sum of budget per year, for currently active grants in DRC", x="Year", y="Budget, with free-y scales", 
                    caption="*Years limited to 2015 - present")

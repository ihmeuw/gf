...
modData[,budget_sum:=sum(budget), by=c("disease", "grant_number")]
...
intData[,budget_sum:=sum(budget), by=c("disease", "grant_number")]
...
int_subset <- setorder(setDT(graphData), -budget)[, head(.SD,3), keyby = "grant_number"]
...
for (k in unique(intData$grant_number)){
  ...
  subdata <- int_data[gf_module%in%int_subset[grant_number==k]$gf_module]
  plot <- (ggplot()+
...
pdf("J:/Project/Evaluation/GF/resource_tracking/uga/visualizations/piecharts/uga_upcoming_modules_sunburst.pdf", height=6, width=9)

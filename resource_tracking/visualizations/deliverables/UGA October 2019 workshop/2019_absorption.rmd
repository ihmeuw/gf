...
# RSSH and program management absorption across grants 

\newpage
```{r rssh}
rssh_mods = c('Community systems', 'Financial systems', "Info systems & M&E", "HR & health workers", "Service delivery", "Nat. health strategies", "Program mgmt")
...
ggplot(rssh_by_int, aes(x=int_factor, y=absorption, fill=abbrev_mod, label=label1))+ 
  ...
  scale_fill_manual(values=wes_palette(7, name="Rushmore1", type='continuous')) + 
  coord_flip() + 
  scale_y_continuous(limits=c(0, 220)) + 
...
melt = cumulative_rssh[, .(abbrev_mod, abbrev_int, int_factor, budget, expenditure)]
  ...
  coord_flip() + 
  scale_y_continuous(labels = scales::dollar) + 
  labs(title=paste0("Cumulative RSSH budget and expenditure \nfor S1 2019"), x="Module", y="Amount ($)", fill="", 
       subtitle="Labels show expenditure amounts")
```
...
stopifnot(nrow(kable_data)==48)
kable(kable_data[31:48, .(abbrev_mod, abbrev_int, start_date, semester, budget, expenditure, absorption)][order(abbrev_mod, abbrev_int, start_date, semester)], col.names=c('Module', 'Intervention', 'Date', 'Semester', 'Budget', 'Expenditure', 'Absorption'), align="l") %>%
  kable_styling(font_size=8)
```

...
gtm = gtm[grant_period%in%c('2018-2018', '2018-2020') | (grant=="GTM-T-MSPAS" & grant_period=="2016-2019" & semester=="Semester 5")]
...
by_grant = gtm[, .(budget=sum(budget, na.rm=T), expenditure=sum(expenditure, na.rm=T)), by=c('grant', 'grant_disease', 'start_date', 'date_range', 'grant_color')]
...
by_module = gtm[, .(budget=sum(budget, na.rm=T), expenditure=sum(expenditure, na.rm=T)), by=c('loc_name', 'grant', 'gf_module', 'abbrev_mod', 'start_date', 'date_range', 'grant_color')]
...
by_intervention = gtm[, .(budget=sum(budget, na.rm=T), expenditure=sum(expenditure, na.rm=T)), by=c('loc_name', 'grant', 'gf_module', 'gf_intervention', 'abbrev_mod', 'abbrev_int', 'start_date', 'date_range', 'grant_color')]
...
I've set up this analysis to include only the most recent PUDR for each grant, because the grant implementation periods vary in Guatemala. The PUDRs in this dataset have start dates between January 2018-April 2019.  

# 1. Overview of 2018-2019 absorption
```{r overview} 
ggplot(by_grant, aes(x=grant, y=absorption, fill=date_range, label=label1))+ 
...
rssh_by_int = gtm[abbrev_mod%in%rssh_mods, .(budget=sum(budget, na.rm=T), expenditure=sum(expenditure, na.rm=T)), by=c('abbrev_mod', 'abbrev_int')]
...
\newpage 
## Findings 
"Financial management" and "program and data quality" stand out as the modules with the highest absorption, and it's notable that program and data quality was able to achieve this even with a large budget allocation. Several important RSSH interventions have not yet recorded any expenditure, namely laboratory systems, surveys, and other HMIS interventions. 

\newpage
# 2. Analyses to support deep dives
## TB laboratory systems and case diagnosis spending 
```{r tb_deep_dive} 
subset = by_intervention[grant=="GTM-T-MSPAS (2016-2019)" & abbrev_mod=="MDR-TB"]
subset[, label2:=dollar(budget)]
ggplot(subset, aes(x=abbrev_int, y=budget, label=label2)) + 
  geom_bar(stat="identity", fill=unique(subset$grant_color)) + 
  theme_bw(base_size=16) + 
  geom_text(size=4, hjust=0) + 
  scale_y_continuous(labels=scales::dollar, limits=c(0, 175000)) + 
  labs(title="MDR-TB budget for GTM-T-MSPAS in most recent PUDR", x="Intervention", y="Budget", caption="*Expenditure was $0 for all modules during this period") + 
  coord_flip() 
```

The TB grant hasn't recorded any expenditure yet, so we can't report on any absorption numbers. However, in the PUDR budget, there is some money allocated to MDR-TB. 

The next step here would be to review the activities in the budget under this module. 
...
```{r bednets_over_time, fig.height=15} 
bednet_ints = c("LLIN: Continuous distribution", "LLIN: Mass campaign") 
bednet_budgets = budgets[abbrev_int%in%bednet_ints, .(budget=sum(budget, na.rm=T)), by='start_date']
...
```{r sigsa1} 
source("C:/Users/elineb/Documents/gf/resource_tracking/visualizations/graphing_functions.r")

#Make funding landscape graph 
funding_landscape("gtm", "hiv", 2010, 2017, altCaption = "*Other DAH data only available until 2017", includeGHE=TRUE)
```

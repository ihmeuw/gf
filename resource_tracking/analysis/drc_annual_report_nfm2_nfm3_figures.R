# DRC 2020 Annual Report Budget Figures - adapted from synthesis figures
...
m <- ggplot(plot_data[loc_name==location & disease=="malaria" & (version_short!="NFM2 FR")],
...
outFilem = paste0(outDir,location, '/cc_fr_comparisons_mal_modules.png')
outFileh = paste0(outDir,location, '/cc_fr_comparisons_hiv_modules.png')
outFilet = paste0(outDir,location, '/cc_fr_comparisons_tb_modules.png')

png(outFilem, height = 6, width = 18, units = "in", res = 300)
m
dev.off()

png(outFileh, height = 6, width = 18, units = "in", res = 300)
h
dev.off()

png(outFilet, height = 6, width = 18, units = "in", res = 300)
t
dev.off()
...
png(outFilee, height = 6, width = 16, units = "in", res = 300)
e
dev.off()
...
#######################################################################
# sum budget across many variables
e_grant_table <- plot_grant_equity[loc_name==location,.(budget=sum(budget, na.rm=TRUE)), by=c('loc_name', 'version', 'label','grant')]
...
e_grant_table <- data.table(dcast(e_grant_table, loc_name + grant + version + Total ~label, value.var = "budget" ))
...
r <- ggplot(plot_rssh[loc_name==location & version!="NFM2 FR"], aes(y=budget, x=version, fill=simplified_mod)) + 
  geom_bar(stat = 'identity') +
  coord_flip()+
...
png(outFiler, height = 8, width = 16, units = "in", res = 300)
r
dev.off()

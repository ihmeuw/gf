...
by_module[, total_budget:=sum(budget, na.rm=TRUE), by='grant']
by_module[, total_expenditure:=sum(expenditure, na.rm=TRUE), by='grant']
...
# What are the highest absorbing modules across countries?
```{r high-performing_mods} 
# Take the 5 highest performing modules per grant on absorption 
# Then, give each of them a value of 1, and collapse to country-level, and to consortia-level. 
# What are the highest performing modules? 
high_performers = data.table() 
for (g in unique(by_module$grant)){
  subset = by_module[order(grant, -absorption)][grant==g, .(loc_name, grant, abbrev_mod, absorption)]
  subset = subset[1:5]
  high_performers = rbind(high_performers, subset)
}

high_performers[, count:=1]
high_performers_by_loc = high_performers[, .(count=sum(count)), by=c('abbrev_mod', 'loc_name')][order(-count)]
high_performers_by_loc[, label:=as.character(count)]
high_performers_all = high_performers[, .(count=sum(count)), by=c('abbrev_mod')][order(-count)]
high_performers_all[, label:=as.character(count)]

ggplot(high_performers_all[!is.na(abbrev_mod)], aes(x=reorder(abbrev_mod, count), y=count, label=label)) + 
  geom_point() + 
  geom_text(size=4, hjust=-0.5) + 
  coord_flip() + 
  theme_bw(base_size=16) + 
  scale_y_continuous(breaks=seq(0, 20, by=2), labels=seq(0, 20, by=2)) + 
  labs(title="What are the highest-performing modules across grants?", y="Number of grants with this module in top 5 highest-absorbing modules", x="Module")
```

# What are the highest absorbing modules, by grant? 
```{r high_performers_by_loc} 

ggplot(high_performers_by_loc[!is.na(abbrev_mod)], aes(x=reorder(abbrev_mod, count), y=count, label=label)) + 
  geom_point() + 
  geom_text(size=4, hjust=-0.5) + 
  coord_flip() + 
  theme_bw(base_size=16) +
...
# What are the highest absorbing interventions across countries?
```{r high_performing_ints} 
# Take the 5 highest performing interventions per grant on absorption 
# Then, give each of them a value of 1, and collapse to country-level, and to consortia-level. 
# What are the highest performing modules? 
high_performers = data.table() 
for (g in unique(by_intervention$grant)){
  subset = by_intervention[order(grant, -absorption)][grant==g, .(loc_name, grant, abbrev_int, absorption)]
  subset = subset[1:5]
  high_performers = rbind(high_performers, subset)
}

high_performers[, count:=1]
high_performers_all = high_performers[, .(count=sum(count)), by=c('abbrev_int')][order(-count)]
high_performers_all[, label:=as.character(count)]

ggplot(high_performers_all[!is.na(abbrev_int) & count>1], aes(x=reorder(abbrev_int, count), y=count, label=label)) + 
  geom_point() + 
  geom_text(size=4, hjust=-0.5) + 
  coord_flip() + 
  theme_bw(base_size=16) + 
  scale_y_continuous(breaks=seq(0, 20, by=2), labels=seq(0, 20, by=2)) + 
  labs(title="What are the highest-performing interventions across grants?", y="Number of grants with this interventions in top 5 highest-absorbing interventions", x="Interventions", caption="*Only showing interventions with counts>1")
```
...
plot_data = all_absorption[gf_module%in%kp_mods & grant_period=="2018-2020" & start_date=="2019-01-01", .(budget=sum(budget, na.rm=TRUE), expenditure=sum(expenditure, na.rm=TRUE)), by=c('abbrev_mod', 'gf_module')]
plot_data[, absorption:=round((expenditure/budget)*100, 2)]

...
governmental = c('COD-M-MOH', 'GTM-M-MSPAS', 'UGA-M-MoFPED', 'UGA-T-MoFPED', 'COD-H-MOH', 'UGA-H-MoFPED', 'COD-T-MOH', 
                 'GTM-T-MSPAS', 'SEN-Z-MOH', 'SEN-S-MOH', 'UGA-S-MoFPED')
civil_society = c('COD-M-SANRU', 'UGA-M-TASO', 'UGA-C-TASO', 'COD-H-CORDAID', 'COD-C-CORDAID', 'GTM-H-HIVOS', 'GTM-H-INCAP', 'COD-H-SANRU', 'COD-T-CARITAS') 
...
high_low_absorption = all_absorption[start_date>="2019-01-01", .(budget=sum(budget, na.rm=TRUE), expenditure=sum(expenditure, na.rm=TRUE)), by=c('grant', 'semester', 'grant_disease', 'abbrev_mod')]
...
high_low_absorption[, mean_absorption:=(mean(expenditure, na.rm=TRUE)/mean(budget, na.rm=TRUE))*100,  by=c('grant', 'semester', 'grant_disease')]
...
```{r rssh_high_low}
rssh_high_low = all_absorption[start_date>="2019-01-01", .(budget=sum(budget, na.rm=TRUE), expenditure=sum(expenditure, na.rm=TRUE)), by=c('grant', 'semester', 'grant_disease', 'disease', 'abbrev_mod')]
rssh_high_low[, absorption:=round((expenditure/budget)*100, 1)]
rssh_high_low[absorption>200, absorption:=200]
rssh_high_low[, mean_absorption:=(mean(expenditure, na.rm=TRUE)/mean(budget, na.rm=TRUE))*100,  by=c('grant', 'semester', 'grant_disease', 'disease')]
...
```{r rq3_1} 
historical_avg = all_absorption[start_date!="2019-01-01", .(hist_budget=sum(budget, na.rm=TRUE), hist_expenditure=sum(expenditure, na.rm=TRUE)), by=c('abbrev_mod')]
historical_avg[, hist_absorption:=(hist_expenditure/hist_budget)*100]
historical_avg[hist_absorption>200, hist_absorption:=200]

only_2019 =all_absorption[start_date=="2019-01-01", .(budget=sum(budget, na.rm=TRUE), expenditure=sum(expenditure, na.rm=TRUE)), by=c('abbrev_mod')]
...
```{r rq3} 
historical_avg = all_absorption[start_date!="2019-01-01", .(hist_budget=sum(budget, na.rm=TRUE), hist_expenditure=sum(expenditure, na.rm=TRUE)), by=c('abbrev_mod', 'loc_name')]
historical_avg[, hist_absorption:=(hist_expenditure/hist_budget)*100]
historical_avg[hist_absorption>200, hist_absorption:=200]

only_2019 =all_absorption[start_date=="2019-01-01", .(budget=sum(budget, na.rm=TRUE), expenditure=sum(expenditure, na.rm=TRUE)), by=c('abbrev_mod', 'loc_name')]
only_2019[, absorption:=(expenditure/budget)*100]
only_2019[absorption>200, absorption:=200]
...
```{r pm1}
pm1 = all_absorption[abbrev_mod=="Program mgmt" & start_date>="2018-01-01", .(budget=sum(budget, na.rm=TRUE), expenditure=sum(expenditure, na.rm=TRUE)), by=c('abbrev_int')] #Collapse across grants
pm1[, absorption:=round((expenditure/budget)*100, 1)]
pm1[, label1:=paste0(absorption, "%")]
pm1[absorption>200, absorption:=200]

pm2 = all_absorption[abbrev_mod=="Program mgmt" & start_date>="2018-01-01", .(budget=sum(budget, na.rm=TRUE), expenditure=sum(expenditure, na.rm=TRUE)), by=c('abbrev_int', 'grant', 'loc_name')] #Collapse across grants
pm2[, absorption:=round((expenditure/budget)*100, 1)]
pm2[, label1:=paste0(absorption, "%")]
pm2[absorption>200, absorption:=200]

 pm1_melt = pm1[, .(abbrev_int, budget, expenditure, label1)]
  pm1_melt = melt(pm1_melt, id.vars=c('abbrev_int', 'label1'), variable.name='variable', value.name='amount')
  ...
  labs(title=paste0("Program management budget and expenditure"), x="Intervention", y="Amount ($)", fill="", 
       subtitle="Labels show expenditure amounts and absorption percentages")
...
ggplot(testing, aes(x=semester, y=absorption, color=abbrev_int, group=abbrev_int)) + 
  geom_point() + 
  geom_line() + 
  theme_bw(base_size=16) + 

...
Subsequent execution of funds (and therefore implementation of activities) has been less than planned. HIV expenditure in Q1 and Q2 of 2018 was only `r standard_round(absorption_by_disease[disease == "hiv", .(mean) ])`% of the original budget across the eight PCE countries. Absorption has varied widely by activity too, with expenditure on some modules such as `r tolower(low_absorption_hiv[1, .(gf_module)])` and `r tolower(low_absorption_hiv[2, .(gf_module)])` not beginning at all in PCE countries. Others, such as `r tolower(high_absorption_hiv[1, .(gf_module)])` and `r tolower(high_absorption_hiv[2, .(gf_module)])` have begun at higher levels in the PCE countries, with a mean of `r standard_round(mean(high_absorption_hiv$mean))`% absorption with `r high_absorb_hiv_country` (the maximum) executing `r standard_round(absorption_by_country_hiv[gf_module %in% high_absorb_hiv_mods, max(mean)])`%. 

Overall execution of funds was higher for TB grants in the PCE countries, which spent `r standard_round(absorption_by_disease[disease == "tb", .(mean)])`% of their budgeted amounts in Q1 and Q2. Again, some modules were able execute more funds than others, with `r tolower(high_absorption_tb[1, .(gf_module)])` achieving a mean of `r high_absorption_tb[1, .(mean)]`% absorption (minimum: `r high_absorption_tb[1, .(min)]`%, maximum: `r high_absorption_tb[1, .(max)]`%) and `r tolower(high_absorption_tb[2, .(gf_module)])` achieving a mean `r standard_round(high_absorption_tb[2, .(mean)])`% absorption (minimum: `r high_absorption_tb[2, .(min)]`%, maximum: `r high_absorption_tb[2, .(max)]`%). 
For malaria, overall execution of funds was higher than HIV but lower than TB, at around `r standard_round(absorption_by_disease[disease == "malaria", .(mean)])`%. `r high_absorption_malaria[2, .(gf_module)]` had the highest absorption across malaria grants (mean: `r high_absorption_malaria[2, .(mean)]`%, minimum:`r high_absorption_malaria[2, .(min)]`%, maximum: `r high_absorption_malaria[2, .(max)]`%), while `r tolower(low_absorption_malaria[1, .(gf_module)])` (mean: `r low_absorption_malaria[1, .(mean)]`%, minimum:`r low_absorption_malaria[1, .(min)]`%, maximum: `r low_absorption_malaria[1, .(max)]`%) and `r tolower(low_absorption_malaria[2, .(gf_module)])` (mean: `r low_absorption_malaria[2, .(mean)]`%, minimum: `r low_absorption_malaria[2, .(min)]`%, maximum: `r low_absorption_malaria[2, .(max)]`%) absorbed very little.

`r overall_low_absorption[1, .(gf_module)]`, `r tolower(overall_low_absorption[2, .(gf_module)])`, and `r tolower(overall_low_absorption[3, .(gf_module)])` had universally low absorption across all PCE countries, with an average of `r standard_round(prepped[gf_module %in% overall_low_absorb_mods, mean(absorption_q1_2018)])`%. 

Table X displays absorption by disease and module.
...
```
 ...
 grant budgets have become more commoditized over time. For example, commodity procurement in Uganda accounted for `r round((grants_12_14[commodity == TRUE, sum(budget)]/grants_12_14[, sum(budget)])*100, 2)`% of all budgets in the 2012-2014 cycle, `r round((grants_15_17[commodity == TRUE, sum(budget)]/grants_15_17[, sum(budget)])*100, 2)`% in the 2015-2017 cycle, and `r round((grants_18_20[commodity == TRUE, sum(budget)]/grants_18_20[, sum(budget)])*100, 2)`% in the 2018-2020 cycle.
...
```
 ...
 For example, `r round(nrow(hist_absorb_by_module[value_mean_historical_absorption < 50])/nrow(hist_absorb_by_module), 2)*100`% of modules have historically had lower than 50% average absorption across all countries and diseases. 
```{r, echo = FALSE }
...
This apparent contradiction is perhaps a reflection of the statement above, that performance indicators that are too distal to immediately respond to lapses in Global Fund resources. The low correlation is empirically supported too; the PCE has observed a correlation of only `r round(cor(absorption_to_kpis$absorption, absorption_to_kpis$achievement_ratio), 1)`% across the eight PCE countries between average absorption and average target achievement by module. 

...
data <- data[,.(loc_name, gf_module, nfm2_approved, nfm2_most_recent_revision)]
...
rssh_data <- rssh_data[,.(loc_name, gf_module, nfm2_approved, nfm2_most_recent_revision)]
...
equity_data <- equity_data[,.(loc_name, gf_module, nfm2_approved, nfm2_most_recent_revision)]
...
p1 <- ggplot(plot_data, aes(y=difference, x=type, fill=type)) +
  ...
  coord_flip()+
  labs(title="Difference between Approved Budget and Most Recent Budget Revision",
...
p1
ggsave("absolute_differences_nfm2award_revision.png", path = out.path, plot=p1, width = 9.5, height=9, units = "in")
...
p2
ggsave("percent_differences_nfm2award_revision.png", path = out.path, plot=p2, width = 9.5, height=9, units = "in")

...
data = allData[data_source %in% c('gos','pudr') & country=='Uganda']

# collapse to module level
byVars = c('grant_number','grant_period','start_date','period','data_source','abbrev_module')
data = data[, .(expenditure=sum(expenditure), budget=sum(budget)), by=byVars]

# if we have a PUDR and GOS in the same grant-quarter, drop the GOS
data[, has_gos:=ifelse(data_source=='gos',1,0)]
data[, has_gos:=max(has_gos), by=c('grant_number','grant_period','start_date')]
data[, has_pudr:=ifelse(data_source=='pudr',1,0)]
data[, has_pudr:=max(has_pudr), by=c('grant_number','grant_period','start_date')]

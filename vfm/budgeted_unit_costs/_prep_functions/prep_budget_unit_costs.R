...
# PURPOSE: Pull cost category and unit cost columns from 
# general detailed budgets. 
# DATE: Last updated August 2019. 
# ----------------------------------------------

prep_budget_unit_costs =  function(file_dir, inFile, sheet_name, language) {
  ...
  master_file_dir = paste0("J:/Project/Evaluation/GF/resource_tracking/_gf_files_gos/", master_file_list$loc_name[i], "/raw_data/")
  ...
  inFile = master_file_list$file_name[i]
  sheet_name = master_file_list$sheet_financial[i]
  language = master_file_list$language_financial[i]
  ...
  # Load/prep data
  gf_data <-data.table(read.xlsx(paste0(file_dir,inFile), sheet=sheet_name, detectDates=TRUE))
  ...
  # 1. Subset columns.
  #-------------------------------------
  #Find the correct column indices based on a grep condition. (Want to keep unit costs and cost input categories)

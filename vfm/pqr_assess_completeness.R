...
expenditure = expenditure[start_date>="2018-01-01" & start_date<"2018-07-01", .(expenditure=sum(expenditure, na.rm=T)), by=c('cleaned_cost_category', 'grant', 'grant_period')]
...
dt = pqr[grant_name == 'COD-M-SANRU']
compare = expenditure[ grant == 'COD-M-SANRU']
...
dt[purchase_order_date >= '2018-01-01' & purchase_order_date <= '2019-01-01', sum(total_cost_order), by = .(product_category)]

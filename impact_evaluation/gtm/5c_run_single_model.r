...
for(v in names(subData)[!names(subData)%in%c('department','date')]) { 
  ...
  if (all(dt5[[v]]>=0)) dt5[, paste0((v), "_jitter"):=get(v)+runif(nrow(dt5), min=0, max=1/100)] #Trying a more heavy-handed jitter to try and resolve zero-variance issue EL 8/6/2019

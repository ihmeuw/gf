...
# ----------------------------------------------
# Bottlenecks in efficienty and effectiveness

actVars = c('ITN_received_cumulative', 'ACT_received_cumulative', 'RDT_received_cumulative')
outVars = c('ACTs_SSC_cumulative', 'ITN_consumed_cumulative', 'RDT_completed_cumulative', 
	'severeMalariaTreated_cumulative', 'SP_cumulative', 'totalPatientsTreated_cumulative')
...
p1 = ggplot(new_means1[lhs %in% actVars & rhs!='date'], 
	...
	labs(y='Activities per Additional Dollar Invested') + 
	theme_bw() + 
	coord_flip()
# ----------------------------------------------

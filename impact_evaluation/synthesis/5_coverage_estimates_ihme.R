...
dt = dt[ ,.(indicator = covariate_name_short, country = location_name,
       sex, year = year_id,
...
dt = dt[sex=='Male']
dt[ ,sex:=NULL]
dt[ ,indicator:='Effective antimalarial coverage']

# change the names of art and rbind
setnames(art, 'year_id', 'year')
...
ggplot(dt, aes(y=mean, x=year, ymin=lower, ymax=upper, color=indicator, fill=indicator)) + 

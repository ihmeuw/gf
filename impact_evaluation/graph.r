...
# * yscale - numeric. what to divide the y-axis by to make numbers more legible
...
# Define function
graph = function(itsOutput=NULL, quarterly=FALSE, title=NULL, ylab=NULL, yscale=1) {
	...
	if (start==end) label1 = label2 = 'Start of Rationalization'
	if (start!=end) label1 = 'Start of Rationalization'
	if (start!=end) label2 = 'End of Rationalization'
	
	# "breaks" (labels) for manual legend
	breaks1 = '95% CI'
	breaks2 = c('Counterfactual', 'Trend', 'Observed Cases')
	colorLab1 = 'Counterfactual'
	colorLab2 = 'Trend'
	colorLab3 = 'Observed Cases'
	colorVals = c(cfColor, pointColor, lineColor) # this relies on alphabetical order
	# ----------------------------------------------------------------------------------------------------------------
	...
	p = ggplot(graphData, aes(x=moyr)) +			
			# add geoms
			geom_ribbon(aes(ymax=upper/yscale, ymin=lower/yscale, fill=breaks1)) + 
			geom_line(aes(y=cf/yscale, color=colorLab1), linetype=2) +
			geom_line(aes(y=trend/yscale, color=colorLab2)) + 
			geom_point(aes(y=observed/yscale, color=colorLab3)) + 
			
			# control colors
			scale_fill_manual('', breaks=breaks1, values=ciColor) + 
			scale_color_manual('', breaks=breaks2, values=colorVals) + 
			...
			geom_segment(aes(x=x, y=y/yscale, xend=xend, yend=yend/yscale), data=annotationLocation, color=annoColor) +
			annotate('text', label=annotation, x=annotationX, y=annotationY/yscale, size=3, vjust=.5, hjust=1) +
			...
			annotate('text', label=label1, x=start, y=max(graphData$observed, na.rm=TRUE)/yscale, size=3, hjust=1, vjust=-.25, angle=90) +
			annotate('text', label=label2, x=end, y=max(graphData$observed, na.rm=TRUE)/yscale, size=3, hjust=1, vjust=-.25, angle=90) +

...
data = untransformed[, c('health_zone','date','malariaDeaths_under5','malariaDeaths_under5_rate','completeness_totalPatientsTreated')]
data[malariaDeaths_under5_rate<0, malariaDeaths_under5_rate:=0]
...
means = data[, .(malariaDeaths_under5_rate=mean(malariaDeaths_under5_rate), 
	lower_pctle=quantile(malariaDeaths_under5_rate, 0.025), upper_pctle=quantile(malariaDeaths_under5_rate, 0.975)), 
...
lmFit = lm(malariaDeaths_under5_rate~intervention*period, data)
summary(lmFit)
...
# histogram
ggplot(data[malariaDeaths_under5_rate<100], aes(x=(malariaDeaths_under5_rate))) + 
	geom_histogram() + 
	facet_grid(intervention_label~period_label, scales='free') 

# traditional DiD graph
ggplot(means, aes(y=malariaDeaths_under5_rate, ymin=lower_pctle, ymax=upper_pctle, x=period_label, color=intervention_label)) + 
	...
	labs(title='Comparison of Health Zones', 
		...
		y='Under-5 Malaria Mortality Rate (per 10,000 population)', x='Period', color='Health Zones', 

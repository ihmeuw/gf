...
# check_explan_power = check_explan_power[!var%in%c('region', 'date')]
...
#   warning("There are some variables with 5 or less data points in this region.")
...
#   pre_jitter_vars = pre_jitter_vars[!pre_jitter_vars%in%c('region', 'date')]
#   
...
#   
#   long = melt(subData, id.vars=c('region', 'date'))
#   
#   #Make graphs 
#   pre_jitter_plot = ggpairs(subData[, c(pre_jitter_vars), with=F], title = paste0("Pre-jitter correlations of all variables in the model for region ", d))
#   jitter_plot = ggpairs(subData[, c(jitter_vars), with=F], title = paste0("Jitter correlations of all variables in the model for region ", d))
#   
#   j_fin_plot =  ggplot(long[variable%in%jitter_fin_vars], aes(y=value, x=date)) + 
#     geom_line() + 
#     facet_wrap(~variable, scales='free') + 
#     labs(title=paste('Time series of jittered financial variables for region ', d), y='Value', x='Date') + 
#     theme_bw()
#   j_act_plot =  ggplot(long[variable%in%jitter_act_vars], aes(y=value, x=date)) + 
#     geom_line() + 
#     facet_wrap(~variable, scales='free') + 
#     labs(title=paste('Time series of jittered activity/output variables for region ', d), y='Value', x='Date') + 
#     theme_bw()
#   
#   pre_j_fin_plot =  ggplot(long[variable%in%pre_jitter_fin_vars], aes(y=value, x=date)) + 
#     geom_line() + 
#     facet_wrap(~variable, scales='free') + 
#     labs(title=paste('Time series of pre-jittered financial variables for region ', d), y='Value', x='Date') + 
#     theme_bw()
#   pre_j_act_plot =  ggplot(long[variable%in%pre_jitter_act_vars], aes(y=value, x=date)) + 
#     geom_line() + 
#     facet_wrap(~variable, scales='free') + 
#     labs(title=paste('Time series of pre-jittered activity/output variables for region ', d), y='Value', x='Date') + 
#     theme_bw()
#   
